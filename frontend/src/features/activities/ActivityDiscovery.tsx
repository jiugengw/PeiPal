...
import { activitiesQueryOptions, DEFAULT_ACTIVITY_RESULTS_LIMIT } from '@/features/activities/api/activityQueries'
...
export function ActivityDiscovery() {
  ...
  const activitiesQuery = useQuery(activitiesQueryOptions({ location, limit: DEFAULT_ACTIVITY_RESULTS_LIMIT }))
  ...
  useEffect(() => {
    if (!selectedActivity || activitiesQuery.isPending || activitiesQuery.isError) return

    const rows = activitiesQuery.data?.activities ?? []
    const stillAvailable = rows.some((row) => row.dedupe_key === selectedActivity.dedupeKey)
    if (!stillAvailable) {
      setSelectedActivity(null)
      setUnavailableNotice('That activity is no longer available. Choose another one below.')
    }
  }, [activitiesQuery.data, activitiesQuery.isError, activitiesQuery.isPending, selectedActivity])
    ...
    <section className="min-h-full bg-[linear-gradient(105deg,var(--muted)_0%,var(--background)_72%)] px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
      <div className="mx-auto grid w-full max-w-[1180px] gap-8 lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-10">
        <div className="min-w-0">
          <header className="mb-6">
            <h1 className="max-w-[16ch] text-4xl font-bold leading-[0.98] tracking-[-0.035em] text-balance text-foreground sm:text-5xl">
              {greetingName ? `Something new for ${greetingName}?` : 'Find something to look forward to.'}
            </h1>
            <p className="mt-4 max-w-[65ch] text-lg leading-relaxed text-foreground">
              Browse nearby activities, or search a neighborhood to narrow things down. Choose one to see the full details.
            </p>
          </header>
          ...
          {unavailableNotice ? (
            <p className="mt-6 rounded-xl border border-input bg-background p-4 text-base font-bold text-foreground" role="alert">
              {unavailableNotice}
            </p>
          ) : null}
        ...
        <aside className="lg:sticky lg:top-8 lg:self-start">
          <SelectedActivityPanel activity={selectedActivity} onClear={clearSelection} />
        </aside>
      </div>
    </section>
  )
