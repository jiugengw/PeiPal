...
describe('ActivityListItem', () => {
  it('does not show the description until expanded', () => {
    render(<ActivityListItem activity={activity()} isSelected={false} onSelect={vi.fn()} />)

    expect(screen.queryByText('A gentle seated class.')).not.toBeInTheDocument()
  })

  it('reveals details without affecting selection when expanded', async () => {
    const user = userEvent.setup()
    const onSelect = vi.fn()
    render(<ActivityListItem activity={activity()} isSelected={false} onSelect={onSelect} />)
  ...
  it('calls onSelect with the full activity when chosen', async () => {
    ...
    render(<ActivityListItem activity={target} isSelected={false} onSelect={onSelect} />)

    await user.click(screen.getByRole('button', { name: /choose this activity/i }))

    expect(onSelect).toHaveBeenCalledWith(target)
  })

  it('shows a written selected state and disables the choose action', () => {
    render(<ActivityListItem activity={activity()} isSelected onSelect={vi.fn()} />)
