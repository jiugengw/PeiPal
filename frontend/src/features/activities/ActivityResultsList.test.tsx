...
function baseProps(overrides: Partial<Parameters<typeof ActivityResultsList>[0]> = {}) {
  return {
    activities: [],
    locationFilter: '',
...
describe('ActivityResultsList', () => {
  it('shows a loading status while the query is pending', () => {
    render(<ActivityResultsList {...baseProps({ query: { isPending: true, isError: false, refetch: vi.fn() } as never })} />)
  ...
  it('offers a retry action on error', async () => {
    ...
    const refetch = vi.fn()
    render(<ActivityResultsList {...baseProps({ query: { isPending: false, isError: true, refetch } as never })} />)

    expect(screen.getByRole('alert')).toHaveTextContent(/could not load activities/i)
  ...
  it('shows a location-clear action in the empty state when a location filter is applied', async () => {
    ...
    render(<ActivityResultsList {...baseProps({ locationFilter: 'Bishan', onClearLocation })} />)

    expect(screen.getByText(/no activities found near "bishan"/i)).toBeVisible()
  ...
  it('shows a plain empty state without a clear action when there is no location filter', () => {
    ...
    expect(screen.getByText(/no activities are available right now/i)).toBeVisible()
    expect(screen.queryByRole('button', { name: /clear location filter/i })).not.toBeInTheDocument()
  })
  ...
  it('renders each activity as a ruled list row', () => {
    render(
      <ActivityResultsList
        {...baseProps({
          activities: [activity({ dedupeKey: 'a', title: 'Senior Yoga' }), activity({ dedupeKey: 'b', title: 'Tai Chi' })],
        })}
      />,
    )
