...
function renderAuthForm() {
  return render(<AppProviders><AuthForm /></AppProviders>)
}
...
describe('AuthForm', () => {
  ...
  it('logs in through Supabase', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /^log in$/i }))
    expect(signInMock).toHaveBeenCalledWith({ email: 'person@example.com', password: 'safe-passphrase' })
    expect(await screen.findByRole('status')).toHaveTextContent(/taking you to setup/i)
  })
  ...
  it('creates a Supabase account and asks for email confirmation', async () => {
    ...
    await user.type(screen.getByLabelText(/email address/i), 'mary@example.com')
    await user.type(screen.getByLabelText(/^password$/i), 'safe-passphrase')
    await user.click(screen.getByRole('button', { name: /create account/i }))
    expect(signUpMock).toHaveBeenCalledWith({ fullName: 'Mary Lim', email: 'mary@example.com', password: 'safe-passphrase' })
