...
function planResponse(status: "awaiting_approval" | "approved" | "shared" | "cancelled") {
...
function renderRow(status: "awaiting_approval" | "approved" = "awaiting_approval") {
  vi.spyOn(fetchClient, "GET").mockResolvedValue({ data: activity, response: new Response(null, { status: 200 }) } as never);
  const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  return render(<QueryClientProvider client={queryClient}><FamilyPlanRow plan={planResponse(status)} olderAdultName="Mary" userId="user-1" /></QueryClientProvider>);
}
...
describe("FamilyPlanRow", () => {
  ...
  it("requires confirmation before sharing an approved plan", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: /share with the family/i }));
    expect(patch).not.toHaveBeenCalled();
    await user.click(screen.getByRole("button", { name: /confirm sharing/i }));

    await waitFor(() => expect(patch).toHaveBeenCalledWith("/api/plans/{plan_id}", { params: { path: { plan_id: 9 } }, body: { status: "shared" } }));
    expect(await screen.findByRole("status")).toHaveTextContent(/shared with the family/i);
  });
