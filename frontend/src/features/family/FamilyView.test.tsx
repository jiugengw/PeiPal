...
describe("FamilyView", () => {
  ...
  it("groups waiting, decided, finished, and cancelled plans", async () => {
    vi.spyOn(fetchClient, "GET").mockResolvedValue({ data: { plans: [plan(1, "coordinating"), plan(2, "completed"), plan(3, "cancelled"), plan(4, "ready")] }, response: new Response(null, { status: 200 }) } as never);
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    render(<QueryClientProvider client={queryClient}><FamilyView /></QueryClientProvider>);

    expect(await screen.findByText("Plan 1: coordinating")).toBeVisible();
    expect(screen.getByText("Plan 2: completed")).toBeVisible();
    expect(screen.getByText("Plan 3: cancelled")).toBeVisible();
    expect(screen.getByText("Plan 4: ready")).toBeVisible();
    ...
    expect(screen.getByRole("heading", { name: /finished/i })).toBeVisible();
