...
describe('useSetupProgress', () => {
  ...
  it('enables dependent setup queries only when their parent record exists', () => {
    ...
    expect(mockedUseQuery.mock.calls[1]?.[0]).toEqual(expect.objectContaining({ enabled: false }))
    expect(mockedUseQuery.mock.calls[2]?.[0]).toEqual(expect.objectContaining({ enabled: false }))
  ...
  it('loads older adults and contacts sequentially and reports completion', () => {
    ...
    mockedUseQuery
      ...
      .mockReturnValueOnce(queryResult({ trusted_contacts: [contact] }) as never)

    const { result } = renderHook(() => useSetupProgress())

    expect(mockedUseQuery.mock.calls[1]?.[0]).toEqual(expect.objectContaining({ enabled: true }))
    expect(mockedUseQuery.mock.calls[2]?.[0]).toEqual(expect.objectContaining({ enabled: true }))
