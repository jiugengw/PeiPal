...
describe('fetchClient', () => {
  ...
  it('adds the current Supabase access token', async () => {
    ...
    const fetchImplementation = vi.fn().mockResolvedValue(Response.json({ activities: [] }))
    ...
    expect(request.url).toBe('https://api.example.test/api/activities?location=Bishan&limit=5')
  })
  ...
  it('omits the authorization header when there is no session', async () => {
    ...
    expect(requestFromFetchCall(fetchImplementation).headers.has('Authorization')).toBe(false)
  })

  it('serializes typed request bodies and path parameters', async () => {
    const fetchImplementation = vi.fn().mockResolvedValue(Response.json({ id: 7 }))
    const client = createApiFetchClient(fetchImplementation)
