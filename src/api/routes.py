...
@router.post(
    ...
    description=(
        "Public on purpose: the person asking is not signed in yet. Codes are "
        "generated by Supabase and delivered by PeiPal's own email for registered "
        "accounts."
    ),
)
def request_sign_in_code(
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="We could not send the code just now. Please check the address or try again in a moment.",
        ) from error
    return {"sent": True, "message": "If an account exists for that address, we sent a sign-in code."}
...
def create_plan(
    ...
    require_scope(user, "plans:write")
