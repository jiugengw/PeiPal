...
@router.post(
    "/auth/sign-in-code",
    response_model=SignInCodeResponse,
    tags=["Families"],
    summary="Email an older adult a sign-in code",
    description=(
        "Public on purpose: the person asking is not signed in yet. Only an "
        "email already added as an older adult receives a code, and the code is "
        "generated by Supabase but delivered by PeiPal's own email."
    ),
)
def request_sign_in_code(
    payload: SignInCodeRequest,
    client: Client = Depends(get_supabase_client),
) -> dict[str, Any]:
    email = str(payload.email).strip().lower()
    profile = older_adult_for_email(client, email)
    if not profile:
        raise HTTPException(
            status_code=404,
            detail="We do not recognise that email address. Ask whoever set up PeiPal for you to add it.",
        )
    try:
        send_sign_in_code(client, email, profile.get("preferred_name") or profile["name"])
    except Exception as error:
        raise HTTPException(
            status_code=502,
